    ...
    fetch('../sounds.json')
      .then(function (res) {
        res.json().then(function (data) {
          this.allSounds = lodash.orderBy(data, 'label')
  ...
  methods: {
    ...
    showSoundOnSelect (sound) {
      if ((this.search !== null && this.search !== '')) {
        ...
        return true
      } else return lodash.includes(sound.categories, this.selectedCategory)
    },
    inputIntoSelectOrNot (sound) {
      if (this.selectedCategory !== 'null' && this.selectedCategory !== null) {
        return (lodash.includes(sound.categories, this.selectedCategory) && (sound.label.includes(this.search.toLowerCase()) || sound.src.includes(this.search.toLowerCase())))
      } else {
